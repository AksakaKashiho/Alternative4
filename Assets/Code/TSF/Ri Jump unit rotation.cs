...
public class RiJumpunitrotation : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            targetRotation = Quaternion.Euler(-45f, 180f, -90f);
            Debug.Log("右");
        }
        else if (x < -0.1f) // x == -1f の厳密な判定から、安全な閾値判定に変更
        {
            targetRotation = Quaternion.Euler(-40f, -15f, -65f);
            Debug.Log("左");
        }
        ...
        {
            targetRotation = Quaternion.Euler(-25f, -90f, 180f);
            Debug.Log("後方"); // ログを修正
        }
